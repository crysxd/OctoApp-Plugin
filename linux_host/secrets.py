    ...
    def _DoConfigMigrationIfNeeded(self, config:Config) -> None:
        ...
        if configPrinterId is not None:
            ...
            if currentPrinterId is not None:
                self.Logger.error("Secrets", f"!! A printer ID was found in the config file but there's already one set in the secrets file? config:{configPrinterId}, secrets:{currentPrinterId}")
            else:
                self.Logger.info("Secrets", "Old printer id found in config file, moving to the secrets file.")
        ...
        if configPrivateKey is not None:
            ...
            if currentPrivateKey is not None:
                self.Logger.error("Secrets", "!! A private key was found in the config file but there's already one set in the secrets file?")
            else:
                self.Logger.info("Secrets", "Old private key found in config file, moving to the secrets file.")
                # Save into our file first, this will throw if there's a problem
