...
from .interfaces import ICommandWebsocketProvider, ICommandWebsocketProviderBuilder, IWebSocketClient, WebSocketOpCode
...
class MqttConnectionContext:
    def __init__(self,
                 ...
                 useTls:bool=False,
                 allowInvalidCert:bool=False,
                 transport:str="tcp",
                 keepAliveSec:int=5,
                 websocketPath:Optional[str]=None) -> None:
...
class IMqttWebsocketProxyConnector:
    def GetConnectionContext(self, args:Optional[Dict[str, Any]], isClosed:Callable[[], bool]) -> Optional[MqttConnectionContext]:
        raise NotImplementedError()


# The actual MQTT proxy websocket class. This bridges an OctoApp websocket to a MQTT broker.
class MqttWebsocketProxy(IWebSocketClient):

    def __init__(self, logger:logging.Logger, connector:IMqttWebsocketProxyConnector, args:Optional[Dict[str, Any]],
    ...
    def _EnsureClientIsDisconnected(self, client:Optional[mqtt.Client]) -> None:
        if client is None:
            return
        try:
    ...
    def _RunThread(self) -> None:
        try:
            connectionContext = self.Connector.GetConnectionContext(self.Args, self._IsClosed)
            if connectionContext is None:
                raise Exception(f"{self._GetLogMsgStart()} No MQTT connection context is available.")
            ...
            self.Logger.info(f"{self._GetLogMsgStart()} connecting to {connectionContext.IpOrHostname}:{connectionContext.Port}...")
            MqttWebsocketProxy.SetupAndConnectMqtt(self.Client, connectionContext)
    ...
    # A helper to setup and connect the mqtt client. This will throw if the connection fails.
    @staticmethod
    def SetupAndConnectMqtt(client:mqtt.Client, context:MqttConnectionContext) -> None:
        if context.UseTls:
            if context.AllowInvalidCert:
        ...
        if context.Transport == "websockets" and context.WebsocketPath is not None:
            client.ws_set_options(path=context.WebsocketPath)

        if context.UserName is not None or context.Password is not None:
            client.username_pw_set(context.UserName, context.Password)

        client.connect(context.IpOrHostname, int(context.Port), keepalive=context.KeepAliveSec)
    ...
    def _OnConnect(self, client:mqtt.Client, userdata:Any, flags:Any, reason_code:Any, properties:Any) -> None:
        ...
        try:
            if reason_code is not None and reason_code.is_failure:
                self._InternalClose(Exception(f"MQTT proxy connection failed: {reason_code}"))
                return
...
class MqttWebsocketProxyProviderBuilder(ICommandWebsocketProviderBuilder):
    ...
    def __init__(self, logger:logging.Logger, connector:IMqttWebsocketProxyConnector):
        self.Logger = logger
        self.Connector = connector
    ...
    def GetCommandWebsocketProvider(self, args:Optional[Dict[str, Any]]) -> Optional[ICommandWebsocketProvider]:
        return MqttWebsocketProxyProvider(self.Logger, self.Connector, args)
...
class MqttWebsocketProxyProvider(ICommandWebsocketProvider):
    ...
    def GetWebsocketObject(self, streamId:int, path:str, pathType:int, context:HttpInitialContext,
        ...
        return MqttWebsocketProxy(self.Logger, self.Connector, self.Args, streamId, path, pathType, context,
