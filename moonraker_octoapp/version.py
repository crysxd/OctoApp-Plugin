...
class Version:
    ...
    def GetPluginVersion(repoRoot:str) -> str:
        ...
        projectFilePath = os.path.join(repoRoot, "pyproject.toml")
        if os.path.exists(projectFilePath) is False:
            raise Exception("Failed to find our repo root project file to parse the version string. Expected Path: "+projectFilePath)
        ...
        if versionLine is None:
            raise Exception("Failed to find a line that starts with '"+expectedVersionKey+"' in project file: "+projectFilePath)
