...
class MoonrakerClient(IMoonrakerClient):
    ...
    @staticmethod
    def Init(logger:logging.Logger, config:Config, moonrakerConfigFilePath:Optional[str], printerId:str, connectionStatusHandler:IMoonrakerConnectionStatusHandler, pluginVersionStr:str, moonrakerDatabase:MoonrakerDatabase):
        MoonrakerClient._Instance = MoonrakerClient(logger, config, moonrakerConfigFilePath, printerId, connectionStatusHandler, pluginVersionStr, moonrakerDatabase)
    ...
    def __init__(self, logger:logging.Logger, config:Config, moonrakerConfigFilePath:Optional[str], printerId:str, connectionStatusHandler:IMoonrakerConnectionStatusHandler, pluginVersionStr:str, moonrakerDatabase:MoonrakerDatabase) -> None:
        self.Logger = logger
        self.Config = config
        ...
        self.ScheduledNotificationsCache:Dict[str,Dict[int,str]] = {}
    ...
    def _OnWsNonResponseMessage(self, msg:Dict[str, Any]) -> None:
        ...
        if method == "notify_status_update":
            ...
            if progressFloat is not None and filePos_CanBeNone is not None:
                self.MoonrakerCompat.OnPrintProgress(progressFloat, filePos_CanBeNone)
    ...
    def DownloadFileForProcessing(self, fileName:str):
        try:
            ...
            while True:
                ...
                if result.HasError():
                    ...
                    if result.ErrorCode == -32602 or result.ErrorStr == "Unauthorized":
                        ...
                        self.Logger.error("!!!! Moonraker auth is required, so you must re-run the OctoApp installer or generate an API key in Mainsail or Fluidd and set it the octoapp.conf. The octoapp.conf config file can be found in /data for docker or ~/.octoapp*/ for CLI installs")
                        time.sleep(10)
...
class MoonrakerCompat(IPrinterStateReporter):
    ...
    def ScheduleNotifications(self, notifications:Dict[int,str]):
    ...
    def OnPrintProgress(self, progress:float, filePos:int) -> None:
        # Only process notifications when ready, aka after state sync.
        if self.IsReadyToProcessNotifications is False:
            return
    ...
    def _InitPrintStateForFreshConnect(self) -> None:
        ...
        self._updatePrinterName()
        if fileName is not None:
            MoonrakerClient.Get().DownloadFileForProcessing(fileName)
