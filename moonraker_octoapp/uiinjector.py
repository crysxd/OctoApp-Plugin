...
class UiInjector():
    ...
    def _FindStaticFilesAndGetHash(self):
        ...
        self.StaticUiJsFilePath = os.path.join(expectedRoot, "octoapp-ui.js")
        self.StaticUiCssFilePath = os.path.join(expectedRoot, "octoapp-ui.css")
        ...
        self.Logger.debug("UI Injector", "Static UI Files Hash: "+self.StaticFileHash)
    ...
    def _DoInject(self, staticHtmlRootPath:str) -> bool:
        ...
        if os.path.exists(indexFilePath) is False:
            self.Logger.info("UI Injector", f"Failed to find index.html at {indexFilePath}")
            return False
    ...
    def _UpdateExistingInjections(self, indexHtmlFilePath:str) -> Tuple[bool, bool]:
        try:
            ...
            if jsTagLocation == -1 or cssTagLocation == -1:
                ...
                if jsTagLocation != -1:
                    self.Logger.error("UI Injector", f"A js tag was found but not a css tag in {indexHtmlFilePath}?")
                if cssTagLocation != -1:
                    self.Logger.error("UI Injector", f"A css tag was found but not a js tag in {indexHtmlFilePath}?")
                return False, False
            ...
            if self.StaticFileHash == currentJsHash and self.StaticFileHash == currentCssHash:
                self.Logger.debug("UI Injector", "Found existing ui tags and the hash matches the current files.")
                return True, False
            ...
            self.Logger.info("UI Injector", "Found existing ui tags but the hash didn't match, so we updated the hash.")
            return True, True
        except Exception as e:
    ...
    def _InjectIntoHtml(self, indexHtmlFilePath:str) -> bool:
        try:
            ...
            if headEndTag == -1:
                self.Logger.error("UI Injector", "Failed to find head tag end in "+indexHtmlFilePath)
                return False
            ...
            if jsIndex == -1 or cssIndex == -1:
                self.Logger.error("UI Injector", "Ui injector created new html but the tags weren't found?")
                return False
            ...
            self.Logger.info("UI Injector", "No existing ui tags found, so we added them")
            return True
        except Exception as e:
            if e is UnicodeDecodeError and "invalid continuation byte" in str(e):
    ...
    def _UpdateSwHash(self, staticHtmlRootPath:str) -> None:
        ...
        try:
            ...
            self.Logger.info("UI Injector", f"Updating the sw.js index.html revision [{currentRevision}] -> [{newRevision}]")
            ...
            self.Logger.info("UI Injector", f"Sw.js [{swJsFilePath}] updated.")
        except Exception as e:
