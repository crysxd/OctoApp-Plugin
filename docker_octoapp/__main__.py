...
if __name__ == '__main__':
    ...
    logger.info("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    logger.info("Starting Docker OctoApp Bootstrap")
    logger.info("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    ...
    try:
        ...
        localStoragePath = os.path.join(dataPath, "octoapp-store")
        ...
        launchConfig = {
            "ServiceName" : "octoapp", # Since there's only once service, use the default name.
        ...
        def signal_handler(sig:Any, frame:Any) -> None:
            logger.info("OctoApp Connect Docker - container stop requested")
            sys.exit(0)
        signal.signal(signal.SIGINT, signal_handler)

        # Get the correct package to launch.
        pyPackage = "bambu_octoapp"
        if mode == CompanionMode.ElegooConnect:
            pyPackage = "elegoo_octoapp"
        elif mode == CompanionMode.ElegooCc2Connect:
            pyPackage = "elegoo_cc2_octoapp"
        elif mode == CompanionMode.Klipper:
            pyPackage = "moonraker_octoapp"
        ...
        if result.returncode == 0:
            logger.info(f"OctoApp Connect Docker - plugin exited. Result: {result.returncode}")
        else:
            logger.error(f"OctoApp Connect Docker - plugin exited with an error. Result: {result.returncode}")

    except Exception as e:
        LogException("Exception while bootstrapping up OctoApp Connect.", e)
